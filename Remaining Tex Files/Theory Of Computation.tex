...
\end{prop}
\begin{proof}
Let $L\in P$, and let $M$ be a poly-time decider for $L$. We construct a poly-time verifier for $L$ as follows. For $\omega \in \Sigma^*$, $M`$ on input $<\omega,0>$ simulates M on $\omega$(ignoring the certificate $O$). $M`$ accepts $<\omega, 0>$ off $M$ accepts $\omega$. Since $M$ decides $L$ in poly-time, $M`$ erifiers members of $L$ in poly-time. So $L \in NP$.
\end{proof}
